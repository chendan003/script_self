    ...
    if saveResult:
        ...
        for i in range(len(y_pred)):
            ...
            try:
                ...
                pass
	    print 'filePathList[i] is %s' % filePathList[i]
            #picPath = filePathList[i].replace('/workspace/data/blued_data/tianyan/office_video_feature/features_all'
             #                               , '/workspace/data/blued_data/office_video_face_larger50').replace('.npy', '.jpg')
            #picPath = filePathList[i].replace('/workspace/data/blued_code/face-feature-api/ava-version-python-little-endian/facex_api_response_office_list_#180531-063140'
             #                               , '/workspace/data/blued_data/office_video_face_extend').replace('.npy', '.jpg')
            picPath = filePathList[i].replace(osp.dirname(filePathList[i]), picDir).replace('.npy', '.jpg')
	    print picPath
            picName = filePathList[i].replace('.npy', '.jpg').split('/')[-1]
