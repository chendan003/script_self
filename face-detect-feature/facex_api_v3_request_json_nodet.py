...
output_base_dir = 'facex_api_resp_json_nodet_blue'

_API_NAMES = ['det', 'age', 'gender', 'feature', 'cluster']

bkt_base_dir = 'http://osjyv5zwq.bkt.clouddn.com'
...
def get_faces(face_dict):
    faces = []
    # for face in face_dict['faces']:
    for face in face_dict:
     #   rect = [int(round(pt)) for pt in face['pts']]
        #rect = [int(round(pt)) for pt in face['rect']]
     ...
     #       [rect[0], rect[3]]
     #   ]
      #  faces.append({'pts': rect_pts})
        faces.append({'pts': face['pts']})

    return faces


def request_facex_api(api_names, input_file, output_dir=None, config_file=None):
    #    print_help_info()
...
#            url = configs['bucket_url'] + '/' + url
            ...
            url = os.path.join(bkt_base_dir, url)
            url_list.append(url)
    else:
	ff = fp.readlines()
	for ur in ff:
...
#             continue
        ...
        #faces = get_faces(face_dict_list[url_idx])
        faces = get_faces(face_dict_list[url])
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        api_names = 'det + feat'
        #input_file = r'C:\zyf\github\caffe_feature_extractor_zyf\face_feature_extractor\results-test1\face_feature.json'
     #  input_file = '../../detect_face/log_video311.json'
#        request_facex_api(api_names, input_file)
        input_file = '/workspace/data/blue/face/face_0.lst.txt'
        output_dir = None  # './'
#        config_file = 'facex_api_config_v2_argus.json'
        config_file = 'facex_api_config_v3_ava.json'
#        config_file = 'facex_api_config_v2_ava_internal.json'
